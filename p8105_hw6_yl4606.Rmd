...
bootstrap_df %>%
  ...
  ggplot(aes(x = estimate)) + geom_density()
```
The distribution of both estimates $\beta_1$ and $\beta_2$ follows approximately normal distribution. The distribution of intercept estimate is a little skewed to the left, which may be related to the frequency with which large outliers are included in the bootstrap sample.
...
```{r}
r_sq = bootstrap_df$adj_r_squared
quantile(r_sq, c(0.025, 0.975))
```

```{r}
log = bootstrap_df$log_b1_b2
