...
I use backward elimination method, which removes variable that have large p-value one by one from the original full model and refit. Until all variables are significant, the process completes. Therefore, I derive the model: **bwt ~ babysex + bhead + blength + delwt + fincome + gaweeks + mheight + mrace + parity + ppwt + smoken** with coefficients listed below: 
...
cv_df = 
  ...
  mutate(
    rmse_linear = map2_dbl(my_reg_mod, test, ~rmse(model = .x, data = .y)),
    rmse_smooth = map2_dbl(reg_1_mod, test, ~rmse(model = .x, data = .y)),
    rmse_wiggly = map2_dbl(reg_2_mod, test, ~rmse(model = .x, data = .y)))
```
